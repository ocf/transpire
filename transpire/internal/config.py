...
class LocalModuleConfig(ModuleConfig, BaseModel):
    config_file: Path = Field(description="The path to the transpire config file within the module")
    ...
    def load_py_module(self, name: str) -> ModuleType:
        ...
        return load_py_module_from_file(
            re.sub("[^A-Za-z0-9_]", "_", self.config_file.stem),
            self.config_file,
            name,
        )
...
class RemoteModuleConfig(ModuleConfig, BaseModel):
    git_url: AnyUrl = Field(description="The URL of the remote git repository the module resides in")
    ...
    def load_py_module(self, name: str) -> ModuleType:
        ...
        if not cache_dir.exists():
            subprocess.check_call([config.git_path, "clone", self.git_url], cwd=modules_cache_dir)
        else:
            subprocess.check_call([config.git_path, "remote", "set-url", "origin", self.git_url], cwd=cache_dir)
            subprocess.check_call([config.git_path, "fetch", "origin"], cwd=cache_dir)
            subprocess.check_call([config.git_path, "reset", "--hard", "@{upstream}"], cwd=cache_dir)
...
class ClusterConfig(BaseModel):
    ...
    modules: dict[str, LocalModuleConfig | RemoteModuleConfig] = Field(description="The list of modules to load")
